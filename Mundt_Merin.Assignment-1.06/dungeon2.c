...
static void makePlayerCharacter(dungeon_t *d){
...
static void makeStairs(dungeon_t *d){
	...
	while(stairCount < 4){
		...
		if(mapxy(x,y) == ter_floor_hall || mapxy(x,y) == ter_floor_room){
			...
			mapxy(x,y) = stairCount % 2 == 0 ? ter_floor_downstairs : ter_floor_upstairs;
			stairCount++;
		}
	}
}

static void clearDungeon(dungeon_t *d){
	free(d->rooms);
...
static uint8_t isCellOccupied(dungeon_t *d, int16_t yPos, int16_t xPos, int ignorePC){
	if(!ignorePC){
...
static pair_xy_t getRandomOpenLocation(dungeon_t *d){
	...
	while(1 == 1){
		...
		if(mapxy(xPos, yPos) == ter_floor_hall || mapxy(xPos, yPos) == ter_floor_room){
			//its a floor
			if(isCellOccupied(d, yPos, xPos,0) == 0){
...
static void makeMonsters(dungeon_t *d){
	d->num_chars = numMonsters;
...
static void setupGame(dungeon_t *d){
	
	makePlayerCharacter(d);
	//game_event_t *ev = newEvent(d, &d->pc, 0);
	//heap_insert(&d->event_heap, &ev);
	//create monsters
	d->gameCharacters = malloc(sizeof(game_character_t) * numMonsters);
	...
	//Add PC Event
	gevents[eventCounter].game_char = &d->pc;
	...
	//Add Monster Events
	for(int i = 0;i < d->num_chars;i++){
		gevents[eventCounter].game_char = &d->gameCharacters[i];
		gevents[eventCounter].turnTime = 0;
		heap_insert(&d->event_heap, &gevents[eventCounter]);
		eventCounter++;
	}
...
static void movePC(dungeon_t *d, int xOffset, int yOffset){
	if(hardnessxy(d->pc.pos.x + xOffset, d->pc.pos.y + yOffset) == 0){
		d->pc.pos.x = d->pc.pos.x + xOffset;
		d->pc.pos.y = d->pc.pos.y + yOffset;
	} 
}

static void printMonsters(dungeon_t *d, int startNum){
	clear();
	refresh();
	move(0,0);
	int line = 1;
	int endNum = MAX_MONSTER_LIST < d->num_chars - startNum	
		? startNum + MAX_MONSTER_LIST - 1
		: d->num_chars - 1;
	for(int i = startNum;i <= endNum;i++){
		move(line,0);
		int diffY = d->gameCharacters[i].pos.y - d->pc.pos.y;
		int diffX = d->gameCharacters[i].pos.x - d->pc.pos.x; 
		printw("%c, %d %s %d %s Dead:%s",
			itohex(d->gameCharacters[i].character_type),
			abs(diffY),
			diffY >= 0 ? "north" : "south",
			abs(diffX),
			diffX >= 0 ? "east" : "west",
			d->gameCharacters[i].dead == 0 ? "No" : "Yes"
			);
		line++;
		refresh();
	}
...
static void showMonsters(dungeon_t *d){
	int scrollIndex = 0;
	int input = -1;
	printMonsters(d, scrollIndex * MAX_MONSTER_LIST);
	while(input != 27){
		input = getch();
		switch(input){
			case KEY_UP:
				...
				break;
			case KEY_DOWN:
				if((scrollIndex + 1) * MAX_MONSTER_LIST < d->num_chars ){
					scrollIndex++;
				}
				printMonsters(d, scrollIndex * MAX_MONSTER_LIST);
				break;
...
static void runPCEvent(dungeon_t *d){
	int finished = 0;
	int newDungeon = 0;
	while(!finished){
		int input = getch();
		switch(input){
			...
			case 'n':
				...
				break;
			case '>':
				if(mapxy(d->pc.pos.x, d->pc.pos.y) == ter_floor_downstairs){
					newDungeon = 1;
				}
				finished = 1;
				break;
			case '<':
				if(mapxy(d->pc.pos.x, d->pc.pos.y) == ter_floor_upstairs){
					newDungeon = 1;
				}
				finished = 1;
				break;
		...
		if(newDungeon){
			...
			if(isCellOccupied(d, d->pc.pos.y, d->pc.pos.x, 1) == 1){
				killCharacter(getCharacterFromCell(d, d->pc.pos.y, d->pc.pos.x));
			}
...
static uint8_t runGameEvent(dungeon_t *d, game_event_t *gevent){
	...
	if(gevent->game_char->isPC == 1){
		...
		if(isErratic(gevent->game_char) && rand() % 2 == 0){
			...
			if(isCellOccupied(d, newPos.y, newPos.x, 0)){
			...
			if(gamechar && gamechar->dead != 1){
				...
				switch (mapxy(x,y)) {
					...
					case ter_floor_hall:
						addch(COORIDORFLOOR);
						break;				
					case ter_floor_downstairs:
						addch('>');
						break;
					case ter_floor_upstairs:
						addch('<');
						break;		
					default:
						addch('*');
