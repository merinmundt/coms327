...
static uint16_t monstersLeft(players_t *pl){
	int monsterCount = 0;
	for(int i = 0;i < pl->num_chars;i++){
		if(!pl->gameCharacters[i].dead)
			monsterCount++;
	}
	return monsterCount;
}
...
static uint8_t runGameEvent(dungeon_t *d, players_t *pl, game_event_t *gevent){
	/*printf("event charx %d, chary %d, chartype %d ispc %d time %d\n", 
		...
		gevent->game_char->characterrain_type::ter_type, gevent->game_char->isPC,
		gevent->turnTime);*/
